...
def when_he_realizado_la_seleccion_resultado1(ctx):
	gen = (node for _path, node in e2e.tree(ctx.app) if node.get_role_name() == 'label' and node.get_text(0,-1).startswith("- La primavera"))
	label = next(gen, None)
	assert label and label.get_text(0, -1).startswith == "La primavera (Vivaldi)", label.get_text(0, -1)
